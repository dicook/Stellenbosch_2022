...
format: 
  revealjs:
    multiplex: true
    footer: "[https://github.com/dicook/Stellenbosch_2022](https://github.com/dicook/Stellenbosch_2022)"
...
## Example data: Palmer penguins {background-image="https://allisonhorst.github.io/palmerpenguins/logo.png" background-size="100px" background-position="96% 1%"}
...
penguins_grid <- penguins_grid %>%
  mutate(pp1 = as.matrix(penguins_grid[,1:4])%*%proj[,1],
         pp2 = as.matrix(penguins_grid[,1:4])%*%proj[,2])
```

```{r eval=FALSE}
ggplot(penguins_grid, aes(x=pp1, y=pp2, colour=pred)) +
  geom_point(alpha=0.8, size=1) +
  scale_color_brewer("", palette="Dark2") +
  theme(aspect.ratio=1, legend.position="none")
ggsave("img/penguins_model.png", width=600, height=600, units = "px")
```

```{r eval=FALSE}
grid_clrs <- brewer.pal(3, "Dark2")[as.integer(penguins_grid$pred)]
animate_slice(penguins_grid[,1:4], col=grid_clrs)
```

Global importance cannot provide a single linear view revealing all aspects of the model.
...
![](img/penguins_model.png){width="100%" height="100%"}
...
- <strong style="color:#aec7b2"> SHAP </strong> is computed at each observation, by examining predictions while varying one variable and holding others fixed at mean, see [Molnar (2022) Interpretable Machine Learning](https://christophm.github.io/interpretable-ml-book/shap.html).
- <strong style="color:#aec7b2"> LIME </strong> examines predictions at each observation from randomly generated new observations in local neighbourhood, see [Mason (2020)](https://numbat.space/post/lime/).
...
<br>
<br>
Exploring the local explanations, with a radial tour, using cheem: [https://ebsmonash.shinyapps.io/cheem_initial/](https://ebsmonash.shinyapps.io/cheem_initial/)
...
For this <strong style="color: #7570B3"> Gentoo </strong> penguin, mistaken to be a <strong style="color: #D95F02"> Chinstrap</strong>, the model used only a small component of `fl`, unlike the other Gentoo but more like the bulk of the Chinstrap penguins. The `fl` component is an important difference between this penguin and others in it's species. 
...
Viewable at [https://dicook.github.io/Stellenbosch_2022/slides.html)](https://dicook.github.io/Stellenbosch_2022/slides.html.
